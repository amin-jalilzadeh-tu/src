# # V2: Modify IDF File for Energy Simulation
...
import numpy as np
import os
import shutil #3.1 file operations
import subprocess #4.1
from geomeppy import IDF
...
idf_file_path   = "C:/Users/aminj/OneDrive/Desktop/EnergyPlus/Minimal.idf"
...
# Global paths (modify these paths according to your file locations)
epwfile  =  os.getenv('EPWFILE') # "pD:/Newfolder3/WeatherData/USA_CO_Golden-NREL.724666_TMY3.epw"
buildings_data_file  = "C:/Users/aminj/OneDrive/Desktop/EnergyPlus/enhanced_table2.xlsx"
...
buildings_df = pd.read_excel(buildings_data_file)
...
# 
# base_values = {
#     "Residential": 3.5,  # Base value for residential areas, in W/m2
#     "Commercial": 5.0,  # Base value for commercial areas, in W/m2
#     "Industrial": 6.0,  # Base value for industrial areas, in W/m2
# }
...
def remove_building_object(idf):
    # Get a list of all 'Building' objects in the IDF file
    building_objects = idf.idfobjects['BUILDING']
    
    # If there are any 'Building' objects, remove them
...
def create_building_block(idf, building_row):
    ...
    # Use the area and a factor of the perimeter to calculate width in a realistic range
    # This method aims to balance the width and length while considering both the area and perimeter
    width = max(area / (perimeter / 4), area**0.5)  # Ensures width is not too small or unrealistic
    length = area / width  # Ensures the area is correctly represented
    ...
    orientation = building_row.get('orientation', 0)  # Orientation in degrees from North
    
    coordinates = [(0, 0), (width, 0), (width, length), (0, length)]  # Rectangle coordinates before rotation
    ...
    if orientation != 0:
        ...
        for x, y in coordinates:
            rotated_x = cos(orientation_rad) * x - sin(orientation_rad) * y
            rotated_y = sin(orientation_rad) * x + cos(orientation_rad) * y
            rotated_coordinates.append((rotated_x, rotated_y))
        coordinates = rotated_coordinates
    
    # Use your method to create a new building block within the IDF
...
# ''''
# building_data = {
#     "Shape Length": 40,  # Perimeter in meters
#     "Shape Area": 180,  # Area in square meters
#     "Facade Height in Meters": 12,  # Building facade height in meters
#     "orientation": 50,  # Building orientation in degrees from North
#     "floor height": 3  # Height of each floor in meters
# }
# idf.intersect_match()
# IDF.view_model(idf)'
# '''
...
def add_ground_temperatures(idf):
    ...
    idf.newidfobject(
        ...
        January_Ground_Temperature=1.03,
        February_Ground_Temperature=2.03,
        March_Ground_Temperature=4.13,
        April_Ground_Temperature=9.30,
        May_Ground_Temperature=13.43,
        June_Ground_Temperature=16.52,
        July_Ground_Temperature=21.62,
        August_Ground_Temperature=22.77,
        September_Ground_Temperature=17.78,
        October_Ground_Temperature=12.55,
        November_Ground_Temperature=7.44,
        December_Ground_Temperature=3.20,
    )
...
def add_HVAC_system_to_zones(idf):   # base sample: 1ZoneDataCenterCRAC_wApproachTemp.idf
...
def update_idf_and_save(buildings_df, base_idf_path,output_dir, idd_path):
...
base_idf_path = "C:/Users/aminj/OneDrive/Desktop/EnergyPlus/Minimal.idf"
update_idf_and_save(buildings_df, base_idf_path, OUTPUT_DIR, iddfile)



get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\IDF3.py')
# 15min 10s for 40 
get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\parallel_2.py')
#5min 50 s for 40
get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\generator.py')
# 6min 58 s for 40 


pip install dask

get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\Dask_Multi.py')
get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\Dask_Multi2.py')
get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\Dask_Multi3.py')
get_ipython().system('python C:\\Users\\aminj\\OneDrive\\Desktop\\EnergyPlus\\Dask_Multi4.py')



import os

print("IDF Directory:", os.getenv('IDF_DIRECTORY'))
print("EPW File:", os.getenv('EPWFILE'))
print("IDD File:", os.getenv('IDDFILE'))
...
def run_script(script_path):
    ...
    # While the process is still running, keep collecting data
    try:
        while True:
            if proc.poll() is not None:  # Check if process has ended
                break
            cpu_times.append(p.cpu_percent(interval=0.1))
            memory_usages.append(p.memory_info().rss)
            time.sleep(0.1)  # Sleep a bit to avoid too much polling
    except psutil.NoSuchProcess:
        pass  # Process has finished before monitoring could start

    # Process outputs and calculate the mean usage if any data was collected
    stdout, stderr = proc.communicate()

    print("Execution Time:", proc.create_time(), "-", time.time())
    print("Average CPU Usage:", sum(cpu_times) / len(cpu_times) if cpu_times else 0, "%")
    print("Peak Memory Usage:", max(memory_usages) / (1024 * 1024) if memory_usages else 0, "MB")

    if stderr:
        print("Errors:", stderr.decode())

script_path = r"C:\Users\aminj\OneDrive\Desktop\EnergyPlus\parallel_2.py"
...
print(f"The code block ran for {elapsed_time} seconds.")
...
# Directory where all the modified_building_XXMeter.csv files are stored
directory = 'D:/Try21'
...
# Get all file names and sort them numerically based on the number in the filename
file_list = [filename for filename in os.listdir(directory) if filename.endswith('Meter.csv')]
sorted_file_list = sorted(file_list, key=extract_number)

# Initialize data frames to store the columns for each new CSV file
df_natural_gas = pd.DataFrame()
df_electricity_facility = pd.DataFrame()
df_electricity_building = pd.DataFrame()
...
for filename in sorted_file_list:
    ...
    # Extract and append the specific column to the corresponding new data frame
    df_natural_gas[filename] = df['NaturalGas:Facility [J](TimeStep)']
    df_electricity_facility[filename] = df['Electricity:Facility [J](TimeStep)']
    df_electricity_building[filename] = df['Electricity:Building [J](TimeStep)']

# After all files have been processed, save the new data frames to new CSV files
df_natural_gas.to_csv('D:\\Try21\\NaturalGas_Facility.csv', index=False)
df_electricity_facility.to_csv('D:\\Try21\\Electricity_Facility.csv', index=False)
df_electricity_building.to_csv('D:\\Try21\\Electricity_Building.csv', index=False)
...
for filename in sorted_file_list:
    ...
    df['TotalEnergy [J](TimeStep)'] = df['NaturalGas:Facility [J](TimeStep)'] + df['Electricity:Facility [J](TimeStep)']

    # Extracting only the required columns
    df = df[['Date/Time', 'TotalEnergy [J](TimeStep)']]
    df.columns = ['Time', 'Total Energy (J)']  # Renaming columns for clarity

    # Determine output filename
    building_number = extract_number(filename)
    output_filename = f'D:/Try21/Building_{building_number}_Report.csv'

    # Save the modified dataframe to a new CSV file per building
    df.to_csv(output_filename, index=False)

print("Energy reports generated for each building.")
...
for filename in sorted_file_list:
    ...
    # Extracting only the required columns
    df = df[['Date/Time', 'NaturalGas:Facility [J](TimeStep)', 'Electricity:Facility [J](TimeStep)', 'TotalEnergy [J](TimeStep)']]
    df.columns = ['Time', 'Natural Gas Consumption (J)', 'Electricity Consumption (J)', 'Total Energy (J)']  # Renaming columns for clarity

    # Determine output filename
    building_number = extract_number(filename)
    output_filename = f'D:/Try21/Building_{building_number}_Energy_Report.csv'
...
def main():
    ...
    future = client.submit(check_env_vars)  # Run on one worker
...
def verify_environment():
    ...
    print(f"IDF_DIRECTORY: {idf_directory}")
    print(f"EPWFILE: {epwfile}")
    print(f"IDDFILE: {iddfile}")

    assert os.path.exists(idf_directory), "IDF directory does not exist"
    assert os.path.exists(epwfile), "EPW file does not exist"
    assert os.path.exists(iddfile), "IDD file does not exist"

    # Optionally list files in the directory for extra verification
    print(f"Files in IDF directory: {os.listdir(idf_directory)}")

# Call the verification function at the start of your worker's processing
verify_environment()
...
# # heating systems
# For defining heating systems in your building's energy model, particularly within an EnergyPlus Input Data File (IDF), there are several essential elements you need to specify to ensure accurate simulation and performance assessment. Here's a concise overview of the minimal things you need to define for heating systems:
# 
# 1. Heating Equipment Type
# Description: Specify the type of heating equipment used, such as a furnace, boiler, heat pump, radiant floor heating, etc.
# Purpose: Determines the method of heat generation and distribution, impacting energy use and comfort.
# 2. Fuel Type
# Description: Identify the fuel source for the heating equipment (natural gas, electricity, oil, etc.).
# Purpose: Affects operating costs, environmental impact, and may influence equipment selection.
# 3. Heating Capacity
# Description: The maximum amount of heat the system can provide, often in BTUs per hour or kW.
# Purpose: Ensures the system can meet the building's heating demand on the coldest days.
# 4. Efficiency Rating
# Description: The efficiency of the heating equipment, represented as AFUE (Annual Fuel Utilization Efficiency) for furnaces and boilers, or COP (Coefficient of Performance) for heat pumps.
# Purpose: Higher efficiency means lower energy consumption and operating costs for the same amount of heat output.
# 5. Heating Setpoint
# Description: The temperature at which the system aims to maintain the indoor environment.
# Purpose: Affects occupant comfort and energy consumption; may vary by time of day or occupancy.
# 6. Control Type
# Description: The type of thermostat or control system used to regulate the heating system's operation.
# Purpose: Determines how the system responds to changes in indoor temperature or occupancy, affecting comfort and efficiency.
# 7. Distribution System
# Description (Optional for Minimal Definition, but Recommended): How the heat is distributed throughout the building, such as through forced air ducts, radiators, or radiant floors.
# Purpose: Affects the efficiency of heat delivery and occupant comfort.
# While this list covers the minimal aspects, understanding and accurately defining these parameters are crucial for an effective heating system model. Depending on the complexity of your project and the specific requirements of the simulation software, you may need to provide additional details or fine-tune these basic parameters to achieve a more accurate and reliable model output.

# # cooling systems
# 
#  Here's a list of essential EnergyPlus objects you'll likely need to include when defining cooling systems in your IDF:
# 
# 1. Cooling Equipment Objects
# Chiller:Electric:EIR, AirConditioner:VariableRefrigerantFlow, or CoolingTower:SingleSpeed: Depending on your specific cooling system (e.g., a chiller, VRF system, or cooling tower), you'll choose the appropriate object to represent the cooling equipment, detailing capacity, efficiency, and operational characteristics.
# 2. Thermostat and Setpoint Objects
# ThermostatSetpoint:DualSetpoint: Allows you to define separate heating and cooling setpoints for a zone, enabling the system to maintain the zone temperature within a specified range.
# ZoneControl:Thermostat: Links a thermostat setpoint to a specific zone, controlling the cooling based on the desired setpoints.
# 3. Air Distribution Objects
# AirLoopHVAC: Represents the ductwork and associated components that distribute cooled air throughout the building.
# ZoneHVAC:EquipmentConnections: Details how cooling equipment is connected to each zone, specifying supply and return paths for the conditioned air.
# 4. Control and Scheduling Objects
# Schedule:Compact: Defines operation schedules for the cooling equipment, including when the equipment is available to run and the desired setpoints for different times of the day or week.
# HVACTemplate:* (optional): Simplifies the definition of common HVAC configurations, including cooling systems, by specifying standard setups and operational parameters.
# 5. Loop Connection Objects
# PlantLoop or CondenserLoop: For systems that use water or another fluid for cooling (like chillers), these objects define the loops through which the cooling medium circulates.
# Branch, Connector:Splitter, Connector:Mixer: Specify the layout and connections of the loops, detailing how the fluid flows through different components of the system.
# 6. Pump Objects
# Pump:ConstantSpeed or Pump:VariableSpeed: For chilled water systems and similar setups, defines the pumps that circulate the cooling medium through the system, specifying operational characteristics like flow rate and pump head.
# 7. Cooling Load Calculation Objects
# Sizing:Zone: Determines the cooling (and heating) loads for individual zones, guiding the sizing of equipment and distribution systems.
# Sizing:System and Sizing:Plant: Provide detailed sizing parameters for the air and plant loops, ensuring that the cooling system is appropriately sized to meet the building's needs.
# 8. Additional Efficiency and Control Objects
# Curve:* objects may be needed to define the performance curves of cooling equipment, showing how efficiency varies with operating conditions.
# This list encompasses the primary objects required to define a cooling system within EnergyPlus. Each object comes with a set of parameters that need to be accurately filled in to reflect the design and operational intent of the cooling system. The complexity of your specific system might necessitate additional objects or more detailed specifications, so always refer to the EnergyPlus documentation for comprehensive guidance on each object's usage and requirements.

# Creating a more comprehensive cooling system in EnergyPlus involves not just adding more components but also ensuring these components are interconnected and configured to reflect the real-world operations and interactions within the HVAC system. In this example, we'll build upon the simple DX cooling setup to include a more detailed configuration. This includes adding a condenser, more refined control strategies, and ensuring the system is fully integrated within an air loop.
# 
# This script assumes you are familiar with Python and the Eppy library for EnergyPlus IDF manipulation. It demonstrates defining a complete cooling system, including air handling units (AHU), DX cooling coils, fans, condensing units, and control strategies.
# 
# 
# 
# 
# 
# For a direct expansion (DX) cooling system scenario where we don't use a cooling tower but instead might have an air-cooled condenser or a simpler condensing unit (since cooling towers are typically used with water-cooled systems), you can define a Condenser object that's more appropriate for a DX system. Let's use a Condensing Unit instead, which is more relevant for such setups. In EnergyPlus, while there isn't a direct "Condensing Unit" object for DX systems, the condenser functionality is often integrated into the DX cooling equipment definitions. However, for illustration, we'll focus on enhancing the DX system's definition to reflect a comprehensive setup.
# 
# Since we're adjusting the setup to not use a cooling tower, let's consider enhancing the DX cooling coil and related components for clarity:

# # Ventilation System

# Defining a comprehensive ventilation system in EnergyPlus involves specifying components that facilitate the controlled exchange of indoor air with outdoor air, ensuring adequate indoor air quality and meeting any specific ventilation requirements. This can include natural ventilation, mechanical ventilation, or a combination of both, along with associated control strategies. Here's how you can define a more detailed mechanical ventilation system using Python and the Eppy library for manipulating EnergyPlus IDF files.
# 
# Key Components of a Ventilation System:
# Outdoor Air System:
# Controller:OutdoorAir and OutdoorAir:Mixer to manage the introduction of outdoor air into the system.
# Ventilation Control:
# Controller:MechanicalVentilation to control the rates of mechanical ventilation based on CO2 levels, occupancy, or pre-defined schedules.
# Air Distribution:
# AirLoopHVAC to represent the ductwork and components that distribute air, including ventilation air, throughout the building.
# Exhaust Systems:
# Fan:SystemModel or specific exhaust fan objects to model the exhaust airflow out of the building or specific spaces.
# Zone Ventilation Requirements:
# DesignSpecification:OutdoorAir for specifying the outdoor air requirements by zone, based on occupancy and space usage.

# # Control Systems
# 

# In EnergyPlus, defining control systems for HVAC operations involves specifying how the heating, cooling, ventilation, and other systems are managed and operated to maintain desired indoor conditions efficiently. The control system components in EnergyPlus are numerous, but here’s a focused list of essential objects you’ll need to include to define a basic control system in your IDF (Input Data File):
# 
# 1. Thermostatic Controls
# ZoneControl:Thermostat: Specifies a thermostat for a particular zone. This object is linked with a thermostat setpoint that can have different settings for heating and cooling.
# ThermostatSetpoint:DualSetpoint: Defines the heating and cooling setpoints for a thermostat, allowing for different target temperatures for heating and cooling operations.
# 2. Scheduling
# Schedule:Compact: Defines schedules for various operations, such as equipment operation times, setpoint temperatures, and occupancy. These schedules are crucial for simulating realistic building operations and ensuring systems operate only when needed.
# 3. Humidity Control
# ZoneControl:Humidistat: Controls humidity levels within a zone by specifying desired humidity setpoints. This object is used in conjunction with HVAC components capable of affecting humidity levels, such as dehumidifiers.
# 4. Ventilation Control
# Controller:MechanicalVentilation: Manages the rates of mechanical ventilation based on CO2 levels, occupancy, or scheduled times, ensuring adequate outdoor air intake for maintaining indoor air quality.
# 5. Operational Strategy Controls
# AvailabilityManager:Scheduled: Determines when specific HVAC components are available to operate, based on a predefined schedule. This can be used to ensure equipment operates only during occupied hours or according to specific energy management strategies.
# AvailabilityManager:NightCycle: Manages HVAC system operation during unoccupied periods to maintain minimum temperatures or pre-condition spaces before occupancy.
# 6. Energy Management System (EMS)
# EnergyManagementSystem:Sensor, EnergyManagementSystem:Actuator, EnergyManagementSystem:Program: These objects provide advanced control capabilities, allowing for custom control strategies beyond standard EnergyPlus controls. They can be used to implement complex algorithms for energy savings, demand response, or integration with renewable energy systems.
# 7. Air Flow Control
# AirLoopHVAC:ControllerList: Assigns controllers to an air loop, defining how airflow is controlled within the system. This includes specifying outdoor air controllers, supply fan speed control, and any bypass or economizer controls.
# 8. System and Equipment Control
# Controller:OutdoorAir: Regulates the amount of outdoor air introduced into the air handling unit for ventilation, based on the current needs of the building.
# SetpointManager:*: A variety of SetpointManager objects are available to dynamically adjust setpoints (for temperature, humidity, etc.) based on different conditions or requirements. These are essential for optimizing system performance and responding to varying loads.
# When setting up control systems in EnergyPlus, it’s essential to ensure that control strategies align with the operational goals of the building, whether it's energy efficiency, occupant comfort, indoor air quality, or a combination of these. Properly defined control systems can significantly impact the simulation's accuracy and the building's real-world performance. Always refer to the specific EnergyPlus documentation for detailed guidance on configuring these control system components.

# For a comprehensive control system in an EnergyPlus model, integrating various control mechanisms is crucial for optimizing the HVAC system's performance, ensuring energy efficiency, and maintaining occupant comfort. We'll cover the setup of thermostatic controls, schedules for HVAC operation, demand-controlled ventilation, and advanced control strategies using the Energy Management System (EMS).
# 
# 1. Thermostatic Controls
# Thermostatic controls maintain desired temperature conditions within zones. They are crucial for both heating and cooling operations.

# 2. HVAC Operation Schedules
# Operation schedules control when various components of the HVAC system are active, contributing to energy savings by ensuring systems run only as needed.

# 3. Demand-Controlled Ventilation (DCV)
# DCV adjusts the ventilation rate based on occupancy or air quality, reducing energy use while maintaining indoor air quality.

# 4. Energy Management System (EMS)
# The EMS allows for custom control strategies beyond standard EnergyPlus controls, enabling sophisticated management of HVAC operations based on dynamic conditions.

# The decision to define multiple Energy Management System (EMS) components in an EnergyPlus model depends on the complexity of your control strategy and the specific requirements of your building's HVAC system. In many cases, a single EMS program might be sufficient for simple control strategies, such as adjusting setpoint temperatures based on occupancy or outdoor temperature conditions. However, for more complex or nuanced control strategies that involve multiple zones, varying conditions, or different HVAC components requiring individualized control actions, you might need to define multiple EMS sensors, actuators, and programs. Here's how to decide:
# 
# When One EMS Program is Enough
# Simple Control Logic: If your control strategy is straightforward, such as uniformly adjusting temperature setpoints across the building based on time of day or occupancy, a single EMS program can suffice.
# Limited Variables: When the control strategy revolves around a limited set of variables (e.g., just temperature or occupancy), it simplifies the logic, reducing the need for multiple programs.
# When Multiple EMS Components are Needed
# Zone-Specific Controls: Buildings with diverse usage patterns across different zones may require unique control strategies for each area, necessitating multiple EMS programs tailored to specific conditions.
# Complex HVAC Systems: Advanced HVAC systems with variable refrigerant flow (VRF), multiple air handling units (AHUs), or dedicated outdoor air systems (DOAS) might require individual control strategies for efficient operation.
# Dynamic Conditions: Responding to a wide range of dynamic conditions (e.g., outdoor weather, indoor air quality, variable occupancy) could require separate sensors and actuators to monitor and adjust different system components.
# Example Scenario
# Imagine a building where the ground floor is commercial (requiring constant cooling due to equipment heat gain) and the upper floors are residential (with varying occupancy and heating/cooling needs). This scenario could benefit from multiple EMS components:
# 
# Ground Floor: An EMS program to manage cooling based on equipment operation schedules and indoor temperature.
# Residential Floors: Separate EMS programs for each residential zone, adjusting heating/cooling based on occupancy sensors and time of day.
# Common Areas: An EMS program to control lighting and ventilation in common areas based on occupancy and ambient light levels.
# Implementation Tips
# Modular Approach: Start with a basic EMS setup and expand as needed. This makes it easier to debug and refine your control strategies.
# Documentation: Keep detailed notes on the purpose and logic of each EMS component to simplify troubleshooting and future modifications.
# Testing and Validation: Use EnergyPlus's simulation and reporting features to validate the performance of your EMS strategies, adjusting as necessary to achieve desired outcomes.
# In summary, whether you need one or multiple EMS components in your EnergyPlus model depends on the specific goals and complexity of your project. Starting simple and adding complexity as required can be an effective strategy to manage the intricacies of advanced building controls.
